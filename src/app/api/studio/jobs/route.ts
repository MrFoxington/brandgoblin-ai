...
export async function POST(request: Request) {
  ...
  if (!prompt && brandId) {
    ...
    if (brand?.output_data) {
      ...
      if (imageType === "logo_concept") {
        prompt = `${baseLogo || `Logo concept for ${brandName}`}. Clean icon / symbol mark, professional quality, symbol only — no text, letters, or numbers.${colors ? ` Color palette: ${colors}.` : ""} On a solid, uniform, pure white background — no gradients, drop shadows, or colored backdrop.`;
  ...
  if (!prompt) {
    ...
    const defaults: Record<ImageType, string> = {
      logo_concept:   "A clean, professional logo concept icon mark for a modern brand, on a solid pure white background.",
