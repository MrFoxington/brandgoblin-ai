...
7. **Vercel Web Analytics wired (same day, follow-up commit).** Fox enabled Analytics in the
   Vercel dashboard, migration run in Supabase, all four commits pushed + deployed and
   verified live (the old claim is gone from production). Dashboard "Enable" alone collects
   NOTHING on Next.js — so added `@vercel/analytics` (^1.5.0) to package.json and
   `<Analytics />` (from `@vercel/analytics/next`) at the end of `<body>` in
   `src/app/layout.tsx`. Fox ran `npm install` on his Mac (the Cowork sandbox registry 403s
   this package, so the lockfile was generated by his npm). `npx tsc --noEmit` clean again.
...
(new), plus `src/components/ComparisonSection.tsx` in the prior commit; follow-up commit:
